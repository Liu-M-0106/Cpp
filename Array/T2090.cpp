...
// 的子数组平均值 是 -1 。 构建并返回一个长度为 n 的数组 avgs ，其中 avgs[i]
// 是以下标 i 为中心的子数组的 半径为 k 的子数组平均值 。 x 个元素的 平均值 是 x
// 个元素相加之和除以 x ，此时使用截断式 整数除法 ，即需要去掉结果的小数部分。
...
int main() {
  ...
  int k = 100000;
  Solution solve;
  vector<int> array = solve.getAverages(nums, k);
  for (auto a : array) {
    cout<<a<<' ';
  }
